...
const RightBookComp: React.FC<RightBookCompProps> = ({ book, publisher }) => {
  ...
  const access = ["publication_date", "publisher", "no_pages"];
  const info_titles = ["Published date", "Publisher", "Pages"];
  ...
  const selectedBooks = Array.isArray(authorBooks)
    ? authorBooks.slice(startIndex, startIndex + booksPerPage)
    : [];
  ...
  return (
    <div className="relative -top-[31em] md:static w-full flex flex-col pb-3 pl-2">
      ...
      <div className="mt-[1em]" id="Details">
        ...
        <p className="pl-7">
          <span>Format</span>
          <span className="ml-5">paperback</span>
        </p>
        <p className="pl-7">
          number of pages:{" "}
          <span className="font-bold ml-7">{book.no_pages}</span>{" "}
        </p>
        <p className="pl-4 font-title text-lg mt-[1em]">ID Numbers</p>
        <p className="pl-7">
          Let&apos;s learn:
          <span className="font-bold ml-3">{book.LibraryId}</span>{" "}
        </p>
        <p className="pl-7">
          ISBN 10:<span className="font-bold ml-7">{book.isbn10}</span>{" "}
        </p>
        <p className="pl-7">
          ISBN 13:<span className="font-bold ml-7">{book.isbn13}</span>{" "}
        </p>
      </div>
              ...
              {totalPages > 1 && (
                <button onClick={handlePrev}>
                  <Image
                    ...
                    alt="back icon"
                    className="pt-9 w-[2.5em] md:w-[3em]"
                    width={70}
                    height={30}
                  />
                </button>
